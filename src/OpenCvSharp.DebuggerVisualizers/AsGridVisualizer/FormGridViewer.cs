...
namespace OpenCvSharp.DebuggerVisualizers
{
    ...
    /// CvMatの中身の値を表示するビューア
    /// </summary>
    public partial class FormGridViewer : Form
    {
        private readonly MatProxyGrid proxy;
        private readonly Scalar[,] data;

        public MatProxyGrid ModifiedProxy 
        {
            get
            {
        ...
        public FormGridViewer(MatProxyGrid proxy)
            : this()
        {
            this.proxy = proxy;
            data = new Scalar[proxy.Rows, proxy.Cols];
        ...
        /// </summary>
        /// <param name="proxy"></param>
        private void InitializeDataGridView(MatProxyGrid proxy)
        {
            // 行数、列数の指定
        ...
        private void InitializeComboBox(MatProxyGrid proxy)
        {
